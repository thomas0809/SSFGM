...
void VariableNode::Init(int num_label)
{
    ...
    belief_product = MatrixUtil::GetDoubleArr(num_label);
    zeros = new int[num_label];
}
...
void VariableNode::BeliefPropagation(double* diff_max, bool labeled_given)
{
    ...
    if (labeled_given && this->label_type == KNOWN_LABEL) 
    {
        for (int y = 0; y < num_label; y++)
            msg[y] = this->state_factor[y];
    ...
    for (int y = 0; y < num_label; y++) 
    {
        belief_product[y] = this->state_factor[y];
        zeros[y] = 0;
    }
    for (int i = 0; i < neighbor.size(); i++) 
    {
        ...
        for (int y = 0; y < num_label; y++) 
        {
            ...
            {
                belief_product[y] *= this->belief[i][y];
            }
            if (belief_product[y] > maxp)
                maxp = belief_product[y];
        }
        if (maxp != 0) // avoid double underflow
        {  
            for (int y = 0; y < num_label; y++)
                belief_product[y] /= maxp;
        }
    }
    for (int i = 0; i < neighbor.size(); i++) 
    {
        ...
        for (int y = 0; y < num_label; y++) 
        {
            if (zeros[y] == 1) 
            {
                msg[y] = (this->belief[i][y] == 0) ? belief_product[y] : 0;
            } 
            else 
            {
                msg[y] = belief_product[y];
                if (this->belief[i][y] != 0)
                    msg[y] = msg[y] / this->belief[i][y];
            }
        }
        NormalizeMessage();
        f->GetMessageFrom(id, msg, diff_max);
...
void FactorNode::BeliefPropagation(double* diff_max, bool labeled_given)
{
    for (int i = 0; i < 2; i++)
    {
        if (labeled_given && ((VariableNode*)neighbor[i])->label_type == KNOWN_LABEL)
        {
            for (int y = 0; y < num_label; y++)
                msg[y] = 0;
...
{
    BeliefPropagation(diff_max, labeled_given);
}
